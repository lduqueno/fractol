__kernel void mandelbrot(__global int* pixels, __global float *data)
{
	...
	float cRe = 0;
	float cIm = 0;
	float constantRe = 1.0 * (x - win_width / 2) / (0.5 * zoom * win_width) + move_x;
	float constantIm = (y - win_height / 2) / (0.5 * zoom * win_height) + move_y - 0.4;
	...
	{
		cIm = 2 * cRe * cIm + constantRe;
		cRe = powRe - powIm + constantIm;
		powRe = cRe * cRe;
		powIm = cIm * cIm;
		iteration++;
	}
